...
uses
  ...
  DataSet.Serialize.Adapter.RESTRequest4D, System.Json, FMX.Edit;
...
procedure TClnicPlusForm.EditButton1Click(Sender: TObject);
begin
edit1.Text:= '';
end;

procedure TClnicPlusForm.EditButton2Click(Sender: TObject);
begin
Edit2.Text:= '';
end;

procedure TClnicPlusForm.EntrarBTNClick(Sender: TObject);
begin
//validação das informações
 if (Edit1.Text = '') or (Edit2.Text = '') then
 raise Exception.Create('Informe o ID e o CPF'); // falha se vazio

 //executamos as requisiçãoes em therad separada
 TTask.Run(procedure
 begin
     Login(Edit1.Text, Edit2.Text); //faz login primeiro

     LoadCliente(Token); //carrega info do ususario
...
begin
 ...
 //data_confirmacao ao invés de dataConfirmacao
  ...
  Edit1.SetFocus; // joga foco do teclado
end;
...
procedure TClnicPlusForm.Login(const ID, CPF: String);
var
  JSON: TJSONObject;
  Resposta: IResponse;
begin
...
try
  ...
  JSON.AddPair('cpf', cpf);
    try
      ...
      if Resposta.StatusCode = 200 then begin //token recebido com suecesso
         // ShowMessage('Sucesso: '+Resposta.Content);
         Token := 'Bearer '+Resposta.Content; //caoncatena 'Bearer' com token recebido
         end
         else begin
           ShowMessage('Falha na autenticação: '+Resposta.Content) //falha de id ou cpf
         end;
         except
          On E:Exception do begin
            ShowMessage('Falha no login: '+E.Message); // falha de red, etc.
          end;
          end;
    finally
